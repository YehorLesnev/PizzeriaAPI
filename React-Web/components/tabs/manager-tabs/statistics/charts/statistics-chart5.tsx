...
const StatisticChart5 = () => {
    ...
    const getSale = async ({dateStart, dateEnd}: any) => {
        ...
        getStatistics({dateStart: formattedStart, dateEnd: formattedEnd, uri: 'OrderDeliveryInfo', entity: selectedValue}).then((res) => {
            setData(res)
        } )
    }
    useEffect(() => {
    ...
    return<div className={"flex flex-col items-center py-28"}>
        <div className={"flex items-center justify-center gap-3 p-2 w-full"}>
            <h1 className={"font-bold text-2xl p-3"}>Order Delivery Info</h1>
            <Select
            ...
            {data &&
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <Area type="monotone" dataKey="deliveryPercentage"  stackId="1" stroke="orange" fill="#F0811D" />
                    </AreaChart>
                </ResponsiveContainer>
            }
...
export default StatisticChart5