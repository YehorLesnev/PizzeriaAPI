...
const StatisticChart3 = () => {
    ...
    const getSale = async ({dateStart, dateEnd}: any) => {
        ...
        getStatistics({dateStart: formattedStart, dateEnd: formattedEnd, uri: 'NumberOfOrders', entity: selectedValue}).then((res) => {
            setData(res)
        } )
    }
    useEffect(() => {
    ...
    return<div className={"flex flex-col items-center py-28"}>
        <div className={"flex items-center justify-center gap-3 p-2 w-full"}>
            <h1 className={"font-bold text-2xl p-3"}>Number Of Orders</h1>
            <Select
            ...
            {data &&
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <YAxis />
                        <Tooltip />
                        <Area  dataKey="numberOfOrders" stackId="1" stroke="#FF4A1C" fill="#FF4A1C" />
                    </AreaChart>
                </ResponsiveContainer>
            }
...
export default StatisticChart3